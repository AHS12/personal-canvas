    ...
    <section>
        <page-header>
            <template slot="options">
                <div class="dropdown">
                    <a href="#" class="nav-link pr-1" role="button" data-toggle="dropdown" aria-haspopup="true"
                        aria-expanded="false">
                    ...
                    <div v-if="isReady" class="dropdown-menu dropdown-menu-right" aria-labelledby="actionDropdownMenu">
                        <a v-if="hasAccess" :href="`/${CanvasUI.canvasPath}/posts/${post.id}/edit`"
                            class="dropdown-item">
                            Edit post
                        </a>
                        <a :href="`/${CanvasUI.canvasPath}/stats/${post.id}`" class="dropdown-item"> View stats </a>
                        <a v-if="hasAccess" @click="makePinnedPost" class="dropdown-item">
                            {{ pinButtonText }}
                        </a>
                    </div>
                </div>
            ...
            <div class="col-xl-8 offset-xl-2 col-lg-10 offset-lg-1 col-md-12 mt-3">
                ...
                <div class="media pt-1 pb-5">
                    <router-link :to="{ name: 'show-user', params: { id: post.user.id } }">
                        <img :src="post.user.avatar || post.user.default_avatar"
                            class="mr-3 rounded-circle shadow-inner" style="width: 50px" :alt="post.user.name" />
                    </router-link>
                    ...
                    <div class="media-body">
                        <p class="my-0">
                            ...
                            <span v-if="post.topic.length">
                                in
                                <router-link :to="{ name: 'show-topic', params: { slug: post.topic[0].slug } }"
                                    class="text-decoration-none">
                                    {{ post.topic[0].name }}
                                </router-link>
                            </span>
                        </p>
                        <span class="text-secondary">{{ moment(post.published_at).format('MMM D, Y') }} — {{
                            post.read_time
                            }}</span>
                    </div>
                </div>
                ...
                <div v-if="post.tags.length">
                    <router-link :key="tag.id" v-for="tag in post.tags"
...
export default {
    ...
    computed: {
        ...
        pinButtonText() {
            return this.post?.isPinned ? 'Unpin This Post' : 'Pin This Post';
        },
    },
